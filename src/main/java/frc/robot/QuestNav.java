...
/**
 * Until proper support for Vendordeps is added, this file should be taken from the QuestNav GitHub
 * repository @ https://github.com/QuestNav/QuestNav This library should be manually updated when
 * necessary with the latest version. Please leave this comment in-tact
 */
...
public class QuestNav {
  ...
  private final DoubleSubscriber questTimestamp =
      nt4Table.getDoubleTopic("timestamp").subscribe(-1.0f);

  /**
   * Subscriber for raw position data from Quest in Unity coordinate system (in getTranslation,
   * Unity's x becomes FRC's -y, and Unity's z becomes FRC's x)
   */
  ...
  private final IntegerSubscriber questFrameCount =
      nt4Table.getIntegerTopic("frameCount").subscribe(-1);

  /** Subscriber for Quest battery percentage */
  private final DoubleSubscriber questBatteryPercent =
      nt4Table.getDoubleTopic("device/batteryPercent").subscribe(-1.0f);

  /** Subscriber for Quest tracking status */
  private final BooleanSubscriber questIsTracking =
      nt4Table.getBooleanTopic("device/isTracking").subscribe(false);

  /** Subscriber for Quest tracking loss counter */
  private final IntegerSubscriber questTrackingLostCount =
      nt4Table.getIntegerTopic("device/trackingLostCounter").subscribe(-1);
  ...
  private final DoubleSubscriber heartbeatRequestSub =
      nt4Table.getDoubleTopic("heartbeat/quest_to_robot").subscribe(-1.0);
  ...
  /**
   * Processes heartbeat requests from the Quest headset and responds with the same ID. This helps
   * maintain connection status between the robot and the Quest. <br>
  ...
  /**
   * Cleans up Quest navigation subroutine messages after processing on the headset. Resets the MOSI
   * value to zero if MISO is non-zero. <br>
   * <b>MUST BE RUN IN PERIODIC METHOD</b>
   */
